...
def _pinned_clock(as_of: date):
    """
    Freeze the clock filter_tenders reads, for the length of one call.
    ...
    PATCHED ON ingest.filters, NOT ON ingest. `ingest` used to be one module and
    `ingest.datetime` was the name filter_tenders resolved; it is a package now,
    and the clock filter_tenders actually reads lives in the submodule that
    defines it. Patching the package would bind a name nothing looks up and
    silently pin nothing — the replay would then run on the wall clock and
    quietly disagree with production on any notice closing near today.
    """
    from ingest import filters as _filters
    original = _filters.datetime
    _filters.datetime = _PinnedDatetime(as_of)
    try:
        yield
    finally:
        _filters.datetime = original
