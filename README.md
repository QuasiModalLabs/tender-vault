...
- **Opportunity-shaping via contract-expiry mining.** The whole system so far is *reactive* — it works with opportunities government has already decided to procure. The highest-value point in the lifecycle is earlier: before the RFP exists, when the requirement can still be shaped. The contracts DB already holds a strong public signal for this — a contract whose delivery period ends in 6-24 months is a near-certain future re-procurement, and I already know the incumbent, the value, and the department. The build is small (a query over existing data plus a `CLAUDE.md` instruction) but the framing matters: this is *not* prediction. It's lead generation feeding human judgment. For each expiring contract, Claude reads the description and generates a provocation — "here's how they're doing this now, here's where the approach may be dated, here's an angle worth a conversation" — rather than a forecast. The goal is never missing a public signal, and interrogating the status quo ("why are they doing it that way? could we help them do it better?"), not pretending to know what government will do. The make-or-break is provocation *quality*: a sharp, specific insight an advisor would actually raise, not generic "have they considered modernizing" slop. Worth proving on existing data before investing in broader signal ingestion (departmental plans, committee reports, RFIs, budget priorities), since if the provocations aren't good, more sources won't fix it.
