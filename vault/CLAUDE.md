...
**Read `extraction_status` before trusting a document is readable.** `extracted` means text is there. `no_text_layer` means it's a scan with nothing to read — there's no OCR, so say that to me rather than reporting the document as empty. `unsupported_type` means no extractor for that format; the file is still listed with its hash and size, so you can tell me a pricing spreadsheet is sitting there unread.
